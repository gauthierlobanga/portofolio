{{-- resources/views/pages/formations/show.blade.php --}}
<?php
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
        <nav aria-label="Fil d'Ariane" class="flex items-center gap-1 text-xs sm:text-sm">
            ...
            <a href="{{ route('formations.index') }}" wire:navigate
                class="rounded-lg px-2.5 py-1.5 text-zinc-500 transition-all duration-200 hover:bg-emerald-50 hover:text-emerald-600 dark:text-zinc-400 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400">Formations</a>
            <svg class="h-4 w-4 shrink-0 text-zinc-300 dark:text-zinc-600" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
            <span class="max-w-xs truncate rounded-lg px-2.5 py-1.5 font-medium text-zinc-900 dark:text-white"
                aria-current="page">{{ $formation->title }}</span>
        </nav>
    </div>

    {{-- ==================== HERO ==================== --}}
    <section
        class="relative flex min-h-[60vh] sm:min-h-[70vh] lg:min-h-[80vh] items-end pb-12 sm:pb-20 lg:pb-32 overflow-hidden bg-zinc-950">
        @if ($formation->hasMedia('cover'))
            <div class="absolute inset-0">
                <img loading="eager" fetchpriority="high" src="{{ $formation->getFirstMediaUrl('cover') }}"
                    alt="{{ $formation->title }}"
                    class="h-full w-full object-cover opacity-40 mix-blend-overlay dark:opacity-30 dark:mix-blend-multiply">
        ...
        <div class="absolute inset-0 bg-linear-to-b from-zinc-950/30 to-transparent"></div>
        <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_bottom_center,rgba(16,185,129,0.15),transparent_60%)]">
        </div>

        <div class="relative z-10 mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8" x-data="{ shown: false }"
            x-intersect="shown = true">
            <div class="flex flex-col items-center text-center lg:items-start lg:text-left">
                {{-- Badge catégorie --}}
                @if ($formation->category)
                    <span
                        class="mb-6 inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-xs sm:text-sm font-medium text-white backdrop-blur-md transition-all duration-300"
                        :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                        <svg class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-400" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                {{-- Titre --}}
                <h1 class="max-w-4xl text-balance text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-extrabold leading-tight tracking-tight text-white font-serif transition-all duration-500 delay-100"
                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                    {{ $formation->title }}
                </h1>

                {{-- Métadonnées --}}
                <div class="mt-6 sm:mt-10 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm sm:text-base text-zinc-200 lg:justify-start transition-all duration-500 delay-200"
                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                ...
                {{-- Bouton d'action --}}
                <div class="mt-10 transition-all duration-500 delay-300"
                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                    <a href="{{ route('contact') }}" wire:navigate
                        class="inline-flex h-14 items-center justify-center border-2 border-emerald-500 bg-emerald-500 px-8 font-semibold text-white transition-all duration-300 hover:bg-emerald-600 hover:border-emerald-600 hover:shadow-lg hover:shadow-emerald-500/30 active:scale-[0.97] text-lg">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        <div class="grid gap-12 lg:grid-cols-12 lg:gap-16">
            ...
            <div class="lg:col-span-8">
                ...
                {{-- Contenu Tiptap --}}
                <div class="fi-prose max-w-none w-full mb-16">
                ...
                @if ($this->galleryImages->count() > 0)
                    <div x-cloak x-data="{ shown: false }" x-intersect.once="shown = true"
                        ...
                        <div x-data="{
                            ...
                            open(index) { this.activeIndex = index;
                    ...
                    @if ($formation->tags && $formation->tags->count() > 0)
                        <div
                            ...
                            <h3
                                ...
                                <span class="h-2 w-2 rounded-full bg-emerald-500"></span> Thématiques</h3>
